...
<ul class="menu-sub">
  @if (isset($menu))
    @foreach ($menu as $submenu)
      ...
      @php
        // Permission check for submenu items (mirrors main menu logic)
        $showSubmenuItem = true;
        if (isset($submenu->permission)) {
          $adminUser = auth('admin-guard')->user();
          $showSubmenuItem = $adminUser && (
            $adminUser->hasRole('super-admin', 'admin-guard') ||
            $adminUser->hasPermissionTo($submenu->permission, 'admin-guard')
          );
        }
      @endphp
